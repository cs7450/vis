...
stretchedChord = function(mmap, rdr) {
  ...
  var π = Math.PI,
    ...
    function arc(r, p, a, dataType) {
    var sign = (dataType == "ingredient" ? 1 : -1);
    return "A" + r + "," + r + " 0 " + +(a > π) + ",1 " + (p[0] + sign*pullOutSize) + "," + p[1];
    }


    function curve(p1,dataType) {
    var sign = (dataType == "ingredient" ? 1 : -1);
  ...
  */  
    ...
    var s = subgroup(this, source, d, i), 
      ...
      targetType = mmap[rdr(d).tname] == undefined ? "null":mmap[rdr(d).tname].data,
      sourceType = mmap[rdr(d).sname] == undefined ? "null":mmap[rdr(d).sname].data;
    
  return "M" + (s.p0[0] + pullOutSize) + "," + s.p0[1] + 
      arc(s.r, s.p1, s.a1 - s.a0, sourceType) + 
      curve(t.p0,targetType) + 
      arc(t.r, t.p1, t.a1 - t.a0, targetType) + 
      curve(s.p0,sourceType) + 
      "Z";
   }//chord
